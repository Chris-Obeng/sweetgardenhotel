...
const heroImages = [
  '/rooms/executive_twin.avif',
  '/rooms/executive_suite.avif',
  '/rooms/executive_double.avif',
];
    ...
    <section ref={containerRef} className="relative h-[100dvh] min-h-[600px] w-full overflow-hidden bg-forest">
      {heroImages.map((image, index) => (
        <img
          key={image}
          src={image}
          alt="Sweet Garden Hotel room"
          className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-[1800ms] ease-out ${activeImage === index ? 'opacity-100' : 'opacity-0'}`}
      ...
      <div className="absolute bottom-8 right-8 lg:right-20 flex-col items-end gap-2 z-20 hidden md:flex">
